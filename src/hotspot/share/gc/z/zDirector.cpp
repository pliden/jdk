...
static double select_gc_workers(double serial_gc_time, double parallelizable_gc_time, double alloc_rate_sd_percent, double time_until_oom) {
  ...
  // More than 10% division from the average is considered unsteady
  const double alloc_rate_sd_percent_threshold = 0.10;
  if (alloc_rate_sd_percent >= alloc_rate_sd_percent_threshold) {
    const double half_gc_workers = ConcGCThreads / 2.0;
    const double boost_gc_workers = half_gc_workers * (1.0 + (alloc_rate_sd_percent - alloc_rate_sd_percent_threshold));

    const double unsteady_gc_workers = MAX3<double>(gc_workers, last_gc_workers, boost_gc_workers);
    log_debug(gc, director)("Select GC Workers (Unsteady), "
                           "AvoidLongGCWorkers: %.3f, AvoidOOMGCWorkers: %.3f, LastGCWorkers: %.3f, BoostGCWorkers: %.3f, GCWorkers: %.3f",
                            avoid_long_gc_workers, avoid_oom_gc_workers, (double)last_gc_workers, boost_gc_workers, unsteady_gc_workers);
...
ZDriverRequest rule_allocation_rate_dynamic() {
  ...
  const double compensate_for_fewer_workers = (ConcGCThreads - actual_gc_workers) * sample_interval;
  const double compensate_for_non_linear_speedup = (actual_gc_workers > last_gc_workers) ? (actual_gc_workers - last_gc_workers) * sample_interval : 0;
  const double time_until_gc = time_until_oom - actual_gc_duration - sample_interval - compensate_for_fewer_workers - compensate_for_non_linear_speedup;
